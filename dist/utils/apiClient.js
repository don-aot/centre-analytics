 ...
 */
export async function trackLogin(apiUrl, accessToken, payload) {
    if (!apiUrl) {
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (axiosError.response) {
                throw new Error(`EAO Analytics recording failed: ${axiosError.response.status} ${axiosError.response.statusText}`);
            }
